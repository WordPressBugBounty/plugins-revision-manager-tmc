...
class Revisions extends IComponent {
	...
	public function _a_manualRevisionAcceptance() {
		...
		if( ! current_user_can( App::i()->settings->getCapabilityForAcceptingChanges() ) ){
			wp_die( esc_html__( 'Sorry but you can not do that', 'rm_tmc' ) );
		}

		if( ! isset( $_REQUEST['post'] ) ){
			wp_die( esc_html__( 'Wrong request. Please try again', 'rm_tmc' ) );
		}

		if( ! in_array( get_post_type( $_REQUEST['post'] ), App::i()->settings->getChosenPostTypesSlugs() ) ){
			wp_die( esc_html__( 'This post type is not supported or revision has been already accepted.', 'rm_tmc' ) );
		}
		...
		if( isset( $_REQUEST[ 'wp_nonce' ] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST[ 'wp_nonce' ] ) ), $this::MANUAL_ACCEPT_REVISION_ACTION_NAME ) ){
			...
			if( ! $resultOfDelete ){
				wp_die( esc_html__( 'There was a problem while clearing the revision from database.', 'rm_tmc' ) );
			}
			...
			wp_die( esc_html__( 'Please try again. Basic authentication failed. You should refresh your session.', 'rm_tmc' ) );
    ...
    public function _a_createLinkedPost() {
    	...
    	if( ! current_user_can( App::i()->settings->getCapabilityForCopyCreation() ) ) wp_die( esc_html__( 'You do not have capability to do this.', 'rm_tmc' ) );

    	//  Authentication.
	    if( isset( $_REQUEST['wp_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['wp_nonce'] ) ), $this::CREATE_LINKED_POST_ACTION_NAME ) ){
		    ...
		    if( $post ){    //  If such post exists
			    ...
			    if( $linkedPostId ){
				    ...
				    wp_die( esc_html__( 'Something went wrong with creation of post duplicate.', 'rm_tmc' ) );

			    }

		    } else {

			    wp_die( esc_html__( 'Wrong post ID', 'rm_tmc' ) );
    ...
    public function _a_ajaxDeleteAllRevisionsCallback() {
        ...
        if( current_user_can( App::i()->settings->getCapabilityForAcceptingChanges() ) ){
	        ...
	        /* translators: %1$s is replaced with the number of removed revisions */
            wp_die( esc_html( sprintf( __( 'Removed %1$s revisions from database.', 'rm_tmc' ), $countDeleted ) ) );

        } else {

            wp_die( esc_html__( 'Sorry, but you are not allowed to do that.',   'rm_tmc' ) );
            ...
            if( $linkedPostId && $linkedPostStatus ){
                ...
                if( current_user_can( 'publish_posts' ) ){
                    ...
                    printf( '<div class="misc-pub-section" style="color: #16a085;"><i class="dashicons dashicons-info"></i> %1$s</div>',
                        esc_html__( 'If you publish this post, it will override its original.', 'rm_tmc' )
                    );

                } else {

                    printf( '<div class="misc-pub-section" style="color: #16a085;"><i class="dashicons dashicons-email"></i> %1$s</div>',
                        esc_html__( 'If you change this post to pending, administrators will receive notification about it.', 'rm_tmc' )
                    );
    ...
    public function _a_displayChangeInTitle( $post ) {
    	...
    	if( $linkedPost = get_post( $this->getLinkedPostId( $post->ID ) ) ){
			...
			if( $linkedPostTitle !== $originalPostTitle ){
				...
				echo sprintf( '<div class="rm_tmc_diff">%1$s</div>', esc_html( wp_text_diff( $linkedPostTitle, $originalPostTitle ) ) );
	...
	public function _a_displayChangeInContent( $post ) {
		...
		if( $linkedPost = get_post( $this->getLinkedPostId( $post->ID ) ) ){
			...
			if( strlen( $linkedPostContent ) && strlen( $originalPostContent ) ){

				if( $linkedPostContent !== $originalPostContent ){

					echo sprintf( '<div class="rm_tmc_diff">%1$s</div>',esc_html( wp_text_diff( $linkedPostContent, $originalPostContent ) ) );
	...
	public function _a_displayChangesInBlockEditor( $postType, $post ) {
		...
		if( $hasTitleChanged || $hasContentChanged ){
			...
			add_meta_box(
				...
				esc_html__( 'Revision differences', 'rm_tmc' ),
        	...
        	if( current_user_can( App::i()->settings->getCapabilityForCopyCreation() ) ){
		        ...
		        if(
			        ...
			        $actions['edit_linked_draft'] = sprintf(
				        ...
				        $linkedPostId,
				        esc_html__( 'Edit connected revision draft', 'rm_tmc' )
			        );
			        ...
			        $actions['show_linked_post'] =  sprintf(
				        ...
				        esc_html__( 'Show original post', 'rm_tmc' )
			        );
			        ...
			        $actions['create_linked_draft'] =   sprintf(
				        ...
				        esc_html__( 'Create revision draft', 'rm_tmc' )
			        );
        	...
        	if( current_user_can( App::i()->settings->getCapabilityForAcceptingChanges() ) ){
        		...
        		if(
			        ...
			        $actions['merge_linked_post'] =  sprintf(
				        ...
				        esc_html__( 'Accept revision', 'rm_tmc' )
			        );
        ...
        if( in_array( $post->post_type, $chosenPostTypes ) ){
            ...
            if( $this->arePostsLinked( $linkedPostId, $post->ID ) ){
	            ...
	            if( $post->post_status === 'future' ){
		            $postStates['scheduled'] = esc_html__( 'Scheduled Revision ✓', 'rm_tmc' );
	            }

                if( $post->post_status === 'pending' ){
                    $postStates['pending'] = esc_html__( 'Revision ✓', 'rm_tmc' );
                }

                if( $post->post_status === 'draft' ){
                    $postStates['draft'] = esc_html__( 'Revision ✓', 'rm_tmc' );
                }
