...
class AcfDifferences extends IComponent {
	...
	public function _a_renderAdditionalFieldHtml( $clonedField ) {
		...
		if( $previousValue !== $clonedField['value'] && ! empty( $previousValue ) ) {
			...
			if( empty( $previousValue ) && ! empty( $clonedField['value'] ) ){
				...
				echo wp_kses_post( sprintf( '<div class="rm_tmc_diff">%1$s</div>', $this->getDisplayOfChanges( $previousValue, $clonedField['value'], $clonedField ) ) );
			}
	...
	public function _a_addScriptsAndStyles() {
		...
		echo '<style>' . PHP_EOL;
		echo esc_attr( sprintf( '.acf-field.rm_tmc_changed_value { border-left-color: %1$s !important; }', App::i()->settings->getAcfChangeMarkColor() ) );
		echo esc_attr( sprintf( '.acf-field.rm_tmc_new_value { border-left-color: %1$s !important; }', App::i()->settings->getAcfNewMarkColor() ) );
		echo '</style>' . PHP_EOL;
