...
class Notifications extends IComponent {
	...
	public function _a_ajaxQuickEmailTestCallback() {
        ...
        if( ! current_user_can( 'manage_options' ) ) wp_die( "Insufficient capabilities" );
        if( isset( $_POST['wpnonce'] ) && ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wpnonce'] ) ), 'wp_rest' ) ) wp_die( "Nonce verification failed" );

		//  ----------------------------------------
		//  Prepare data
		//  ----------------------------------------

		$emailTarget    = array_key_exists( 'emailTarget', $_POST ) ? sanitize_email( wp_unslash( $_POST['emailTarget'] ) ) : '';
		$emailSubject   = array_key_exists( 'emailSubject', $_POST ) ? sanitize_text_field( wp_unslash( $_POST['emailSubject'] ) ) : '';
		$emailContent   = array_key_exists( 'emailContent', $_POST ) ? apply_filters( 'the_content', wp_kses_post( wp_unslash( $_POST['emailContent'] ) ) ) : '';
