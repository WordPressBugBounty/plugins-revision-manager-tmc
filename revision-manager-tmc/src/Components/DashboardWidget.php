...
class DashboardWidget extends IComponent {
    ...
    protected function getPostsToRevision( $postTypes = null ) {
		...
		//  This might be a slow query - but is necessary 🤔
    ...
    public function _a_displayOfDashboardWidget() {
        ...
        printf( '<p><strong>%1$s</strong></p>', esc_html__( 'Revisions to accept', 'rm_tmc' ) );
        ...
        if( empty( $posts ) ){
            ...
            foreach( $posts as $post ){ /** @var WP_Post $post */
                ...
                echo wp_kses_post( sprintf( '<li><a href="%1$s" class="dashicons-before dashicons-arrow-right" title="%2$s">%2$s</a></li>', get_edit_post_link( $post ), $post->post_title ) );
        ...
        printf( '<p><strong>%1$s</strong></p>', esc_html__( 'Tools', 'rm_tmc' ) );
        ...
        echo wp_kses_post( sprintf( '<p>%1$s</p>', $buttonObj->getDisplay() ) );
...
JS;
        ...
        echo wp_kses_post( sprintf( '<p class="wp-clearfix"><a class="alignright" href="%1$s">%2$s</a></p>', admin_url( 'options-general.php?page=rm_tmc_settings' ), __( 'Settings', 'rm_tmc' ) ) );
